...
// Initialize Gemini with API key(s) - supports comma-separated keys for rotation
const apiKeys = process.env.GEMINI_API_KEYS || process.env.GEMINI_API_KEY;  // Support both old and new env var
if (!apiKeys) {
  console.error('ERROR: GEMINI_API_KEYS environment variable is missing. Server cannot start without valid API key(s).');
  console.error('Set GEMINI_API_KEYS=key1,key2,key3 in your .env file for key rotation');
  process.exit(1);
}

try {
  initializeGemini(apiKeys);
  console.error('Gemini AI initialized successfully with key rotation');
} catch (error) {
...
async function main() {
  ...
  console.error("═══════════════════════════════════════════════════════════");
  console.error("🛡️  Ethics Check MCP Server Started Successfully!");
  console.error("═══════════════════════════════════════════════════════════");
  console.error("📡 Transport: stdio (for Claude Desktop / Cursor integration)");
  console.error("🌐 For HTTP/Web UI: run 'node server.js' instead (port 3000)");
  console.error("═══════════════════════════════════════════════════════════");
}
